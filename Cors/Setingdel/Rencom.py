...
print (Fore.GREEN + " [!] Для изменения данных. Откройте файл в программе для редактирования данных. \n [?] Найти файл можно по этому пути" + Fore.YELLOW + " [ Cors/Setingdel/ ] " + Fore.GREEN + "откройте файл Coming.py \n" )
input()
...
def open_file():
    file_path = filedialog.askopenfilename(title='Выбор файла', filetypes=(('Coming', '*.py'), ('Все файлы', '*.*')))
    if file_path:
        text_fild.delete("1.0", END)
        text_fild.insert("1.0", open(file_path, encoding='utf-8').read())
        
def save_file():
    file_path = filedialog.asksaveasfilename(filetypes=(('Coming', '*.py'), ('Все файлы', '*.*')))
...
fiel_menu.add_command(label="Открыть Coming.py", command=open_file)
